...
function buildInstructions(): string {
  return [
    ...
    "Follow these steps:",
    "",
    "1. Call `check_health` to check the MCP server version and Hertwill API reachability.",
    "   - Report the rate-limit budget: remaining requests for both the public and authenticated buckets.",
    "2. Call `list_import_list` (page 1) to get a summary of your current import list (total count, sample items).",
    "3. Call `get_sync_jobs` (page 1) to check recent sync job status.",
    "4. Present a consolidated diagnostic report with:",
