...
describe("get_product handler", () => {
      ...
      http.get(`${BASE_URL}/v1/products/:id`, () =>
        HttpResponse.json(
          detailFixture({
            variations: [
              variation,
              { ...variation, id: 2, name: "Blue" },
              { ...variation, id: 3, name: "Green" },
            ],
          }),
        ),
      ),
  ...
  it("Test 4 — 429 upstream", async () => {
    mockServer.use(
      http.get(`${BASE_URL}/v1/products/:id`, rateLimitedResponse429(4)),
    );
    const handler = createGetProductHandler(buildTestDeps());
    const result = await handler({ product_id: 42 } as never);
  ...
  it("Test 5 — 503 upstream", async () => {
    mockServer.use(
      http.get(`${BASE_URL}/v1/products/:id`, serverErrorResponse503()),
    );
    const handler = createGetProductHandler(buildTestDeps());
    const result = await handler({ product_id: 42 } as never);
  ...
  it("Test 7 — description wrapped in <untrusted_supplier_content>", async () => {
    mockServer.use(
      http.get(`${BASE_URL}/v1/products/:id`, () =>
        HttpResponse.json(
          detailFixture({ description: "<script>evil</script>" }),
        ),
      ),
    );
    const handler = createGetProductHandler(buildTestDeps());
    ...
    expect(sc.description).toMatch(
      /<untrusted_supplier_content product_id="42"/,
    );
  });

  it("Test 8b — keyless caller: null price does not crash, surfaces pricing hint", async () => {
    mockServer.use(
      http.get(`${BASE_URL}/v1/products/:id`, () =>
        HttpResponse.json({
          ...detailFixture({ price: null, sale_price: null }),
          meta: {
            request_id: "req-test",
            pricing: {
              authenticated: false,
    ...
    expect(sc.price).toBeNull();
    expect(sc.pricing).toEqual({
      included: false,
      message: "Set HERTWILL_API_KEY to receive wholesale pricing.",
    });
    expect(result.content[0].text).toMatch(/price unavailable/);
